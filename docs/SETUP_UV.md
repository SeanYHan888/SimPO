...
Recommended prebuilt method (auto-resolve exact wheel from the FlashAttention release assets):

```bash
uv run python scripts/install_prebuilt_flash_attn.py
# Optional: only print the resolved URL
uv run python scripts/install_prebuilt_flash_attn.py --print-url
```

The installer script resolves wheel compatibility from the active runtime (`torch`, python tag, and cxx11abi).
...
If mismatch is detected, reinstall with:
